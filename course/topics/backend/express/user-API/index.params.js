...
app.get('/api/users', (req, res) => res.json(ok('Users listing succeeded.', users.map(({ username,password }) => ({ username, password })))))
...
app.put('/api/users', jsonBodyParser, (req, res) => {
    ...
    if (!username || !password) return res.json(ko('User update action failed.', 'Invalid username and/or password.'))
...
app.delete('/api/users', jsonBodyParser, (req, res) => {
    ...
    if (!username || !password) return res.json(ko('User delete action failed.', 'Invalid username and/or password.'))
